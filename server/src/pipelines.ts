...
export function defaultPipeline(projectId: string): Pipeline {
  return {
    ...
    steps: [
      ...
      { id: 'st-2', kind: 'command', laneId: 'ln-1', command: 'npm run build', description: 'Build' },
      { id: 'st-3', kind: 'command', laneId: 'ln-1', command: 'npm test', description: 'Run tests' },
      {
        id: 'st-4',
        kind: 'agent',
        laneId: 'ln-2',
      ...
      { id: 'st-5', kind: 'human', laneId: 'ln-3', description: 'Approval', errorReturnToLaneId: 'ln-1' },
    ],
    updatedAt: '',
  };
}
