...
import { DomainEventJson, EventFrameJson, PublishResponseJson, WireStage } from './wire';
import {
  EVENTS_TRANSPORT,
...
describe('EventsClient', () => {
  ...
  it('folds SSE frames into oneof-shaped events', () => {
    ...
    transport.emit({
      ...
      eventType: 'cardMoved',
      body: { cardId: 'T-1', from: 'new', to: 'review' },
    });

    expect(seen).toHaveLength(1);
    expect(seen[0].cardMoved).toEqual({ cardId: 'T-1', from: 'new', to: 'review' });
  });
  ...
  it('maps command DTOs onto the action envelope before posting', async () => {
    ...
    await client.publish({
      projectId: 'P-1',
      requestPipelineRun: { pipelineId: 'PL-1', cardId: 'T-1' },
    });
    ...
    expect(posted).toEqual([
      {
        path: 'action',
        body: { type: 'update', on: 'card', projectId: 'P-1', body: { id: 'T-1', stage: 'review' } },
      },
      {
        path: 'action',
        body: {
      ...
      },
      {
        ...
        body: {
          ...
          on: 'pipeline',
          projectId: 'P-1',
          body: { pipelineId: 'PL-1', cardId: 'T-1' },
        },
      },
      {
        path: 'action',
...
describe('EventsClient (gateway)', () => {
  ...
  it('publishes every command to the one server', async () => {
    ...
    await client.publish({
      projectId: 'alpha',
      requestCardMove: { cardId: 'T-1', toLane: WireStage.STAGE_REVIEW },
    });
