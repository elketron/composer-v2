...
describe('restart snapshot folding', () => {
  it('restores projects, cards, planning state, and automation before live events', () => {
    ...
    events.emit(
      wireEvent(
        'automationToggled',
        { pipelineId: 'PL-1', laneId: 'ln-2', on: false },
        'P-1',
      ),
    );
    TestBed.tick();
    ...
    expect(board.automation().isOn('PL-1', 'ln-2')).toBe(false);
    ...
    events.emit(
      wireEvent(
        'cardLaneMoved',
        { cardId: 'T-1', pipelineId: 'PL-1', toLaneId: 'ln-4' },
        'P-1',
      ),
    );
    TestBed.tick();
    expect(board.cardsById().get('T-1')?.laneId).toBe('ln-4');
  });
});
