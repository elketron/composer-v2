...
| D13 | 2026-09-07 | **The server owns project-directory selection.** The in-app directory browser reads server-native paths through `GET /directories`; Electron no longer opens an OS-native folder dialog. | A Windows desktop attached to a server inside WSL must select Linux paths visible to the server, not unrelated Windows paths visible to Electron. |
